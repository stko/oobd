...
//  NVIC_Configuration();
    ...
    extern xQueueHandle Led1Queue;
	  ...
	  (Led1Queue = xQueueCreate(QUEUE_SIZE_LED, sizeof(uint16_t))))
	DEBUGUARTPRINT("\r\n*** LedQueue created ***");
    extern xQueueHandle Led2Queue;
    if (pdPASS ==
	  (Led2Queue = xQueueCreate(QUEUE_SIZE_LED, sizeof(uint16_t))))
  ...
  /** USARTx configuration -----------------------------------------------------
    ...
    */ 
	...
	BTM222_UartAutobaudControl = 0;
		...
		USART1_BAUDRATE_115200;
	    break;
	case 7:
	    
		/* Initialize USART1 with next possible baudrate of BTM222 */ 
		USART_InitStructure.USART_BaudRate =
		USART1_BAUDRATE_230400;
	    break;
	case 8:
	    
		/* Initialize USART1 with next possible baudrate of BTM222 */ 
		USART_InitStructure.USART_BaudRate =
		USART1_BAUDRATE_460800;
	    break;
	case 9:
	    
		/* fallback to default baudrate */ 
		USART_InitStructure.USART_BaudRate =
		USART1_BAUDRATE_DEFAULT;
