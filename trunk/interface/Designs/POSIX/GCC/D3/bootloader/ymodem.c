 ...
 */
static int32_t
Receive_Packet(uint8_t * data, int32_t * length, uint32_t timeout)
{
...
int32_t Ymodem_Receive(uint8_t * buf)
{
	...
	file_size[FILE_SIZE_LENGTH], *file_ptr, *buf_ptr;
	...
	for (packets_received = 0, file_done = 0, buf_ptr = buf;;) {
	    switch (Receive_Packet
		    (packet_data, &packet_length, NAK_TIMEOUT)) {
	    case 0:
		errors = 0;
		...
		default:
			...
			if (packets_received == 0) {
				...
				if (size > (FLASH_SIZE - 1)) {
				    /* End session */
				    printf
					("Filesize %d is too big for flash size %d\n",
					 size, FLASH_SIZE);
				    Send_Byte(CA);
				    Send_Byte(CA);
				    return -1;
 ...
 */
void
Ymodem_PrepareIntialPacket(uint8_t * data, const uint8_t * fileName,
			   uint32_t * length)
{
    uint16_t i, j;
    uint8_t file_ptr[10];
 ...
 */
void
Ymodem_PreparePacket(uint8_t * SourceBuf, uint8_t * data, uint8_t pktNo,
		     uint32_t sizeBlk)
{
 ...
 */
uint8_t
Ymodem_Transmit(uint8_t * buf, const uint8_t * sendFileName,
		uint32_t sizeFile)
{
    uint8_t packet_data[PACKET_1K_SIZE + PACKET_OVERHEAD];
