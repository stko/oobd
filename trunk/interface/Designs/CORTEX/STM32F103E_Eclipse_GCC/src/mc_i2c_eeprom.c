			...
			uint16_t * NumByteToRead)
{
    ...
    sEETimeout = sEE_FLAG_TIMEOUT;
	    ...
	    (sEE_I2C, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED))
	...
	*pBuffer = I2C_ReceiveData(sEE_I2C);
	...
	sEETimeout = sEE_FLAG_TIMEOUT;
		...
		(sEE_I2C, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED))
	...
	sEE_LowLevel_DMAConfig((uint32_t) pBuffer,
			       (uint16_t) (*NumByteToRead),
			       sEE_DIRECTION_RX);
		       ...
		       uint8_t * NumByteToWrite)
{
    ...
    printser_string("\r\n Send EEPROM address send succesfully");
...
#ifdef sEE_M24C08
    printser_string("\r\n sEE_M24C08 data send");

    /*!< Send the EEPROM's internal address to write to : only one byte Address */
    I2C_SendData(sEE_I2C, WriteAddr);
    printser_string("\r\n sEE_M24C08 data send successfully");

#elif defined(sEE_M24C64_32)
    printser_string("\r\n sEE_M24C64_32 data send");

    /*!< Send the EEPROM's internal address to write to : MSB of the address first */
    I2C_SendData(sEE_I2C, (uint8_t) ((WriteAddr & 0xFF00) >> 8));
    printser_string("\r\n sEE_M24C64_32 data send successfully");
    ...
    sEE_LowLevel_DMAConfig((uint32_t) pBuffer,
			   (uint8_t) (*NumByteToWrite), sEE_DIRECTION_TX);
		     ...
		     uint16_t NumByteToWrite)
{
	...
	if (NumOfPage == 0)
	    ...
	    /* Start writing data */
	    printser_string("\r\npBuffer");
	    printser_string(pBuffer);
	    printser_string("\r\nWriteAddr");
	    printser_uint16ToHex(WriteAddr);
	    printser_string("\r\nsEEDataNum");
	    printser_uint16ToHex(sEEDataNum);
	...
	if (NumOfPage == 0)
		...
		sEE_WritePage((uint8_t *) (pBuffer + count),
			      (WriteAddr + count),
			      (uint8_t *) (&sEEDataNum));
...
uint32_t sEE_WaitEepromStandbyState(void)
{
	...
	/*!< Send EEPROM address for write */
	I2C_Send7bitAddress(sEE_I2C, sEEAddress,
