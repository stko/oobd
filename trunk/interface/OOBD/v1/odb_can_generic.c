...
void print_telegram(portBASE_TYPE msgType, void *data, printChar_cbf printchar) {
...
void bus_param_can_generic_Print(portBASE_TYPE msgType, void *data,
	...
	if (args->args[ARG_CMD] == PARAM_INFO) {
		...
		case VALUE_PARAM_INFO_BUS_ERROR:
			...
			} else if (VALUE_PARAM_INFO_BUS_ERROR_DEL == args->args[ARG_VALUE_2]) {
...
portBASE_TYPE bus_param_can_generic(param_data * args) {
	...
	case FBID_BUS_SPEC:
		...
		break;
	default:
		createCommandResultMsg(FBID_BUS_GENERIC, ERR_CODE_OS_UNKNOWN_COMMAND,
