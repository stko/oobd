...
public class ScriptengineLua extends OobdScriptengine {
	...
	public void run() {
										...
										new Message(
												...
												new Onion(
														""
																...
																+ "'data':'"
																+ Base64Coder
																		.encodeString(getString(0))
																+ "'}")),
										+(getInt(1) * 12) / 10);

						if (answer != null) {
							Logger.getLogger(ScriptengineLua.class.getName())
									.log(Level.INFO,
		...
		if (scriptFileName == null) {
			Properties props = Core.getSingleInstance().getSystemIF()
					.loadProperty(FT_PROPS, "enginelua.props");
