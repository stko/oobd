...
public class Core extends OobdPlugin implements OOBDConstants, CoreTickListener {
    ...
    public Onion requestParamInput(Onion msg) {
        ...
        try {
            ...
            Message answerMsg = this.getMsgPort().sendAndWait(new Message(Core.getSingleInstance(),
                    UIHandlerMailboxName, thisOnion), -1);
            if (answerMsg != null) {
    ...
    public void userAlert(String msg) {
        try {
            requestParamInput(new Onion("{" + "'param' : [{ " + "'type':'alert',"
                    + "'message':'" + Base64Coder.encodeString(msg) + "'"
                    + "}]}"));
