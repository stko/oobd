...
static int addattr32(struct nlmsghdr *n, size_t maxlen, int type,
		     __u32 data)
{
...
static int open_nl_sock()
{
    ...
    setsockopt(fd, SOL_SOCKET, SO_SNDBUF, (void *) &sndbuf,
	       sizeof(sndbuf));

    setsockopt(fd, SOL_SOCKET, SO_RCVBUF, (void *) &rcvbuf,
	       sizeof(rcvbuf));
...
static int do_get_nl_link(int fd, __u8 acquire, const char *name,
			  void *res)
{
			  ...
			  struct req_info *req_info)
{
	...
	if (req_info->bittiming != NULL) {
	    addattr_l(&req.n, 1024, IFLA_CAN_BITTIMING,
		      req_info->bittiming, sizeof(struct can_bittiming));
	}

	if (req_info->ctrlmode != NULL) {
	    addattr_l(&req.n, 1024, IFLA_CAN_CTRLMODE,
		      req_info->ctrlmode, sizeof(struct can_ctrlmode));
	}
