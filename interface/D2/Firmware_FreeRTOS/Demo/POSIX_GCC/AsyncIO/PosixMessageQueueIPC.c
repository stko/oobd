...
static void prvPosixIPCInterruptHandler(int signal, siginfo_t * data,
					void *pvParam);
/*---------------------------------------------------------------------------*/
		    ...
		    void *pvContext)
{
    ...
    hPipeHandle =
	mq_open(pcPipeName, O_RDWR | O_CREAT | O_NONBLOCK,
...
portLONG lPosixIPCSendMessage(mqd_t hPipeHandle, xMessageObject xMessage)
{
    ...
    if (0 != hPipeHandle) {
	if (0 ==
	    mq_send(hPipeHandle, (const char *) &xMessage,
		    sizeof(xMessageObject), 1)) {
	    lReturn = pdTRUE;
	} else {
				 ...
				 xMessageObject * pxMessage)
{
    ...
    lBytesReceived =
	mq_receive(hPipeHandle, pcBuffer, MAX_MESSAGE_SIZE, &uiPriority);
    for (lBytesToProcess = 0; lBytesToProcess < lBytesReceived;
	 lBytesToProcess += sizeof(xMessageObject)) {
...
void vPosixIPCEmpty(mqd_t hPipeHandle)
{
    ...
    unsigned int uiPriority;
    while (-1 !=
	   mq_receive(hPipeHandle, pcBuffer, MAX_MESSAGE_SIZE,
		      &uiPriority));
}

/*---------------------------------------------------------------------------*/

void prvPosixIPCInterruptHandler(int signal, siginfo_t * data,
				 void *pvParam)
{
	...
	if (NULL != vMessageInterruptHandler) {
	    while ((pdTRUE ==
		    lPosixIPCReceiveMessage(hPipeHandle, &xRxMessage))
		   && (lMessagesReceived++ < MAX_NUMBER_OF_MESSAGES)) {
	...
	printf
	    ("NULL parameter: Failed to install Rx interrupt handler: %d.\n",
	     errno);
    }
}
