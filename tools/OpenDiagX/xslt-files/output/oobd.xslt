...
<!--
<xsl:if test="ENUM"><xsl:if test="ENUM"><xsl:for-each select="EnumMember"><xsl:apply-templates select="ENUM" /></xsl:for-each></xsl:if>dummy=0}
-->
<!--
id0x<xsl:value-of select="./HighPID"/><xsl:value-of select="./LowPID"/> = { ti = "<xsl:value-of select="Name"/>", sv = {
